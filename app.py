...
# ============================================================
#  ghost_packet (sync version - aapnar async er moto)
# ============================================================
def ghost_packet(player_id, nm, secret_code, key, iv):
    """
    Async version ke sync e convert kora hoyeche.
    CDX.py te CrEaTe_ProTo ebong GeneRaTePk sync version thakte hobe.
    """
    fields = {
        1: 61,
        2: {
...
def _build_ghost_packet(proto_bytes, key, iv):
    """Fallback packet builder - CDX er EnC_PacKeT use kore"""
    try:
...
def ERML(open_id, access_token, version="2.126.18"):
    ...
    while True:
        try:
            ...
            if not all_accounts:
                ...
                # 10 সেকেন্ড wait করে আবার চেক করবে
                time.sleep(10)
                continue
            ...
            # অ্যাকাউন্ট থ্রেড চালু
            for uid, password, region in all_accounts:
                thread = threading.Thread(target=rb, args=(uid, password, region), daemon=True)
                thread.start()
                time.sleep(0.05)
            
            # Main wait loop - accounts শেষ হলেও এখানে থাকবে
            while True:
                time.sleep(10)
                
                # যদি সব থ্রেড মরে যায় (কিছু ভুল হলে), আবার check
                if ab == 0 and not rf:
